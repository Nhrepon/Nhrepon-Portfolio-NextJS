...
const SkillList = () => {
  ...
  return (
    <section className="bg-gradient-to-b from-indigo-200 to-gray-200 dark:from-gray-900 dark:to-gray-800 w-full">
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="max-w-7xl mx-auto py-20 sm:px-6 lg:px-8">
        ...
        <motion.div variants={containerVariants} className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-4">
          {
            skillList && skillList.map((skill) => (
              <motion.div
              variants={itemVariants}
              className="flex flex-col items-center justify-between p-2 shadow-md border-1 border-gray-300 rounded bg-linear-to-r from-green-200 to-indigo-100 relative hover:scale-105 hover:shadow-lg transition-all duration-500 ease-in-out cursor-pointer"
                key={skill._id}
              >
